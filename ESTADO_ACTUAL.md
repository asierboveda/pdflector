...
prueba física seguía pendiente durante esa observación.

**Actualización 2026-09-26:** se instaló una build con la corrección de
proyección de tinta y el propietario confirmó que el trazo siguió la punta del
stylus tanto en vertical como en horizontal. Es una observación manual de
alineación, sin medida de latencia o continuidad. El repintado nativo por
evento y la transición Wet→Dry no se evaluaron y siguen sin verificar. Véase
`docs/benchmark-results.md` para el alcance de esta comprobación.
...
El workspace contiene cuatro crates (`Cargo.toml`):
...
target Android explícito (`Cargo.toml`).
...
- Clientes síncronos para Ollama, Groq y Gemini
  (`crates/pdf_core/src/ai.rs`). El consumidor debe sacarlos del hilo
  de UI.
...
(`crates/pdf_android/src/lib.rs`). El proceso mantiene un `Reader` como
estado principal y cuatro modos de UI definidos en
`crates/pdf_android/src/reader/mod.rs`:
...
de arXiv (`crates/pdf_android/src/reader/mod.rs`,
...
(`crates/pdf_android/src/reader/library.rs`). Existen vistas de rejilla
y lista, búsqueda, ordenación, filtros de estado y portadas generadas en un
worker (`crates/pdf_android/src/reader/library_state.rs`,
...
La implementación está en `crates/pdf_android/src/gpu/pipeline.rs` y
`crates/pdf_android/src/gpu/pipeline.rs`. La geometría de tinta se
dibuja con tiras de triángulos y discos con antialiasing, usando buffers scratch
reutilizados (`crates/pdf_android/src/gpu/pipeline.rs`).

#### Entrada, zoom y tinta

La entrada distingue tacto y stylus, consume muestras históricas del lápiz y
normaliza presión y tiempo (`crates/pdf_android/src/input/dispatch.rs`,
`crates/pdf_android/src/input/stylus.rs`). La máquina de gestos conecta:
...
Estas rutas están activas en `crates/pdf_android/src/input/motion.rs` y
`crates/pdf_android/src/reader/tools.rs`. Las anotaciones se almacenan en
...
referenciados con `include_str!` (`crates/pdf_android/src/lib.rs`). El
...
estado de herramientas (`crates/pdf_android/src/persist.rs`). Las
anotaciones usan el sidecar SQLite de `pdf_core` y se guardan fuera del hilo de
UI (`crates/pdf_android/src/reader/anotaciones.rs`).
...
1. El encabezado de `crates/pdf_android/src/lib.rs` conserva un resumen del
   rediseño de 2026-08, cuando los controles de tinta se habían retirado. Se ha
   marcado como histórico porque la ruta de tinta y resaltado volvió a
   integrarse y permanece conectada. La disponibilidad del código está
   verificada; la observación de alineación en TCL consta arriba, mientras que
   otros aspectos de UX siguen pendientes.
...
3. La ruta causal del bolígrafo mantiene una única secuencia `Vec<InkSample>`
   para el trazo activo. Al finalizar, `finish()` construye además los puntos
   `(f32, f32)` que consume `Stroke`; por tanto, el modelado no duplica puntos
   por muestra durante el gesto, pero el recorrido completo sí asigna memoria
   para la salida final (`crates/pdf_android/src/ink/causal.rs`,
   `crates/pdf_android/src/reader/tools.rs`). El `Vec` de `ToolGesture.points`
   corresponde al resaltador.
...
8. `gl_str` recorre el puntero devuelto por `glGetString` sin una guarda nula
   (`crates/pdf_android/src/gpu/shaders.rs`). Esto es un riesgo técnico,
...
9. `PageCache::clear` vacía páginas y contadores, pero conserva los campos
   ...
   (`crates/pdf_android/src/cache.rs`). No se observó un fallo asociado.
