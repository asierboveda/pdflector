...
//! highlight-by-gesture → annotation set → SQLite sidecar round-trip. Keeps
//! the pure units (selection.rs, annotations.rs) honest against MuPDF's real
//! span boxes.
...
use pdf_core::{
    Annotation, AnnotationSet, Document, Gesture, RenderEngine, highlight_under_gesture,
    smooth_polyline,
};
